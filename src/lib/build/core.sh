...
orchestrate_build() {
  ...
  load_env_for_detection() {
    ...
    if [[ -f ".env" ]]; then
      ...
      # Use printenv to avoid bash function pollution (BASH_FUNC_*)
      printenv | grep -v "^_" | grep -v "^PWD=" | grep -v "^SHLVL=" | grep -v "^HOME=" | grep -v "^PATH=" | sed 's/\$/\\$/g' > ".env.runtime"
  ...
  detect_all_services() {
    ...
    # Legacy Optional services (kept for backward compatibility)
