...
# Source .env.runtime if it exists (generated by nself start)
# This ensures all variables required by docker-compose.yml are present
if [[ -f "$PROJECT_ROOT/.env.runtime" ]]; then
  set -a
  source "$PROJECT_ROOT/.env.runtime"
  set +a
fi
